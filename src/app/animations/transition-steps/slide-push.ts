...
export abstract class SlidePush {
  ...
  public static downUp(timeLeave = 1500, timeEnter = 1500): AnimationMetadata[] {
    return [
      ...
      group([
        query(':leave', [
          animate(timeLeave + 'ms ease-out', style({bottom: '100%'}))
        ]),
        query(':enter', [
  ...
  public static upDown(timeLeave = 1500, timeEnter = 1500): AnimationMetadata[]{
    ...
    group([
      query(':leave', [
        animate(timeLeave + 'ms ease-out', style({top: '100%'}))
      ]),
      query(':enter', [
  ...
  public static leftRight(timeLeave = 1500, timeEnter = 1500): AnimationMetadata[]{
    return [
      ...
      group([
        query(':leave', [
          animate(timeLeave + 'ms ease-out', style({left: '100%'}))
        ]),
        query(':enter', [
  ...
  public static rightLeft(timeLeave = 1500, timeEnter = 1500): AnimationMetadata[]{
    return [
      ...
      group([
        query(':leave', [
          animate(timeLeave + 'ms ease-out', style({ right: '100%' }))
        ]),
        query(':enter', [
